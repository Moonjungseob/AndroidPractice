package com.busanit501.androidlaptest2.ch10_dialog_permission
...
//        setContentView(R.layout.activity_ch10_main)
        ...
        val requestPermissionLauncher = registerForActivityResult(
            ...
            if (isGranted) {
                Toast.makeText(this@Ch10MainActivity, "권한이 승인됨, 후처리 콜백 요청", Toast.LENGTH_LONG)
                    .show()
            } else {
                Toast.makeText(this@Ch10MainActivity, "권한이 승인 안됨, 후처리 콜백 요청", Toast.LENGTH_LONG)
                    .show()
            }
        }
        ...
        binding.ch10sampleBtn.setOnClickListener {
            ...
            val toast = Toast.makeText(
                this@Ch10MainActivity,
                "토스트 메세지 출력 또는 사라질 경우 후처리 동작 확인",
                Toast.LENGTH_LONG
            )
        ...
        binding.ch10dateBtn2.setOnClickListener {
            ...
            DatePickerDialog(this@Ch10MainActivity, object : DatePickerDialog.OnDateSetListener {
                override fun onDateSet(p0: DatePicker?, year: Int, month: Int, dayofMonth: Int) {
                    ...
                    Toast.makeText(
                        this@Ch10MainActivity,
                        "년도 : ${year}년, 월 : ${month + 1}월 , 일 : ${dayofMonth}일",
                        Toast.LENGTH_LONG
                    ).show()
...
// 추가사항 , 메뉴 목록 추가
                ...
                val objectListener = object : DialogInterface.OnMultiChoiceClickListener {
                    override fun onClick(p0: DialogInterface?, which: Int, isChecked: Boolean) {
                        Log.d(
                            TAG,
                            "선택한 점심 메뉴 : ${items[which]}  ${if (isChecked) "선택됨" else "선택해제됨"}"
                        )
                        if (isChecked) {
...
//                setMultiChoiceItems(items, booleanArrayOf(false,false,false,false),objectListener)
            ...
            }.setCanceledOnTouchOutside(false)//알림 창 바깥부분 터치 시 알림창 끄는 옵션
        ...
        //resources -> raw -> mp3 음원 재생해보기
        binding.ch10musicBtn.setOnClickListener {
        ...
        binding.ch10vibrateBtn.setOnClickListener {
            val vibrator = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                val vibraorManager = this.getSystemService(Context.VIBRATOR_MANAGER_SERVICE)
                        as VibratorManager
                vibraorManager.defaultVibrator
            } else {
