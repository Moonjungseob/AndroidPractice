...
class MyDecoration(val context:Context) : RecyclerView.ItemDecoration() {
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        //이미지 사이즈 계산
        //리소스에서 이미지 가져오기
        val drawable : Drawable? = ResourcesCompat.getDrawable(context.resources,R.drawable.test,null)

        //실제 이미지 크기
        val drawableWidth = drawable?.intrinsicWidth
        val drawableHight = drawable?.intrinsicHeight
        Log.d("lsy", "실제 이미지의 크기 계산 drawable?.intrinsicWidth : $drawableWidth" + "drawable?.intrinsicHeight : $drawableHight")

        //이미지가 그려질 위치 계산 -> 정 중앙에 배치
        //drawableWidth?/div(2) 실제 이미지의 크기를 2로 나눈값을 정수로 표현
        val left = width /2 - drawableWidth?.div(2) as Int
        val top = height /2 - drawableHight?.div(2) as Int
        Log.d("lsy", "출력할 좌표 left : $left, " + "top : $top")
