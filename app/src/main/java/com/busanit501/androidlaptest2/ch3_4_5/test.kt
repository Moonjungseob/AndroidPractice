...
//늦게 초기화 놀이 2
//laze 뒤에 블록 부분은 해당 상수가 최초로 이용될 때 사용됨
val data : String by lazy{
    println("val data : String by laze : 확인중")
    "오늘 뭐 먹죠?"
}

//함수내부
fun main(){
    //선언만 해도 가능 초기화를 조금 늦게 해도 됨
    println("fun main 실행")
//    var name4 : String
//    var name = "이상용"
//    println("이름 : " + name)
...
//println(data2)
    println("힘이 없네, 잠도 오고, 잠이 계속오네 어쩌지, ㅠㅠ 그냥 이야기함, 오해 금지 : $data2")
...
//    val lunchMemberParty = showLunchMember("재범쌤, 동진쌤")
    ...
    //배열에서 기본형 선언시 선언과 할당을 동시에 기초 타입의 배열을 생성하기.
    ...
    when (data5) {
        ...
        20,30 -> println("2,3번의 숫자 타입입니다.")
        in 1..10 -> println("1이상 10이하의 숫자 타입입니다.")
        else -> {
            println("해당범위에 없는 타입입니다.")
        }
    }
    //when 이용해서 표현식으로 사용하기
    var data6 = 100
    val result = when{
        data6 <= 0 -> "해당 데이터는 0이하입니다."
        data6 >= 10 -> "해당 데이터는 10 이상입니다."
        else -> {
            "지정된 범위의 타입이 아닙니다."
        }
    }
...
println()
    ...
    for ((index,value) in data7.withIndex()){
        print("data7의 index : data7[value] : ${index} : ${data7[index]}" )
        if(index !== data7.size -1)
            print(", ")
        }
